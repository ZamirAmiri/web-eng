...
def update(request, a_code, c_code, id):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('Cancelled') is None:
            cancelled = flight.cancelled
        else:
            cancelled = int(request.POST.get('Cancelled'))

        if request.POST.get('On time') is None:
            on_time = flight.on_time
        else:
            on_time= int(request.POST.get('On time'))

        if request.POST.get('Delayed') is None:
            delayed = flight.delayed
        else:
            delayed = int(request.POST.get('Delayed'))

        if request.POST.get('Diverted') is None:
        ...
        arg_request['total'] = arg_request['cancelled'] + arg_request['on_time'] + arg_request['delayed'] + arg_request[
            'diverted']
